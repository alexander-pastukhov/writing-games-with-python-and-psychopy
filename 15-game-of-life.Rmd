...
```{r echo=FALSE, fig.align='center', out.width='50%'}
knitr::include_graphics("images/game-of-life-neighbors.png")
```
...
```{python echo=FALSE}
icell_row, icell_col = 2, 2
print(grid[(icell_row - 1):(icell_row + 2), (icell_col - 1):(icell_col + 2)].astype(int))
```
...
```{python echo = FALSE}
irel_row, irel_col = np.meshgrid(np.arange(-1, 2), np.arange(-1, 2))
print(grid[irel_row + icell_row, irel_col + icell_col].astype(int))
```
...
```{python echo = FALSE}
print("Neighbor count for the example cell: ", grid[irel_row + icell_row, irel_col + icell_col].sum())
```
...
```{python echo = FALSE}
neighbors = np.zeros(grid.shape)
for icell_row in range(1, grid.shape[0] - 1):
  for icell_col in range(1, grid.shape[1] - 1):
    neighbors[icell_row, icell_col] = grid[irel_row + icell_row, irel_col + icell_col].sum() - grid[icell_row, icell_col]

...
## A random mole
Drawing all three moles served as a practical exercise with loops but in a real game we need to shown only one _random_ target at a time. We could create the three targets as before and draw one of them. However, later on we would like to _change_ the color of target to indicate that the player did hit it, so it is simpler (if a bit wasteful) to create a single mole every time we need one. 

For this, define one [constant](#constants) with a list of three colors that you used and another one with three horizontal locations (the vertical is the same, so we do not need to worry about it). Next, randomly pick which target out of three you want to create, i.e., we need to generate an _index_ of the target. You can do it either via [random.randrange()](https://docs.python.org/3/library/random.html#random.randrange) or via [random.choice()](https://docs.python.org/3/library/random.html#random.choice) building the range yourself via [the function with the same name]() you have just learned about (remember to organize your imports alphabetically). Store the index in a variable with a meaningful name^[itarget? imole?] and use it in combination with constants to create the target. Then, you need to draw that single target in the main loop.
...
Now is time to update and structure you code. Here is a approximate outline:
```python
"""Document your file
"""
...
## Repeating trials
You already know how to [repeat](#range) the same code many times. Decide on number of trials / rounds (define this as a constant) and repeat the single round that many times. Think about what code goes inside the loop and what should stay outside.
...
## Exit strategy
I hope that you used a small number of trials because (on my advice, yes!) we removed the possibility to exit the game via the **escape** key. To put it back, we will replace _both_ [wait()](https://psychopy.org/api/clock.html#psychopy.clock.wait)  calls with [waitKeys()](https://psychopy.org/api/event.html#psychopy.event.waitKeys) function from [event](https://psychopy.org/api/event.html#psychopy-event-for-keypresses-and-mouse-clicks) module. It has `maxWait` parameter that by default is set to infinity, which we can replace it with the duration we require. If a player does not press a key, it will work just the way the [wait()](https://psychopy.org/api/clock.html#psychopy.clock.wait) did. If a player presses a key (allow only `"escape"`!), it means that they want to abort the game (the only option at the moment). Thus, assign the returned value to a temporary variable (`keys`?) and check whether it is equal to `None`^[Confusingly, if no key was pressed, [getKeys()](https://psychopy.org/api/event.html#psychopy.event.getKeys) returns an empty list but [waitKeys()](https://psychopy.org/api/event.html#psychopy.event.waitKeys) returns `None`.]. If it is not equal to `None`, [break](#break) out of the loop!

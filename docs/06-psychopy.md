...
The `# drawing stimuli` chunk goes inside the main loop either before^[My personal preference is to draw first but in most cases it makes no difference.] or after the keyboard check. Organize the latter also as a separate code chunk with its own brief comment.

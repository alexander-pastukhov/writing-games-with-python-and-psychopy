...
\NormalTok{Variable name (name tag on the box) should be meaningful, it can start }\ControlFlowTok{with}\NormalTok{ letters }\KeywordTok{or}\NormalTok{ \_ }\KeywordTok{and}\NormalTok{ can contain letters, numbers, }\KeywordTok{and}\NormalTok{ \_ symbol but }\KeywordTok{not}\NormalTok{ spaces, tabs, special characters, etc. Python recommends (well, actually, [insists](https:}\OperatorTok{//}\NormalTok{www.python.org}\OperatorTok{/}\NormalTok{dev}\OperatorTok{/}\NormalTok{peps}\OperatorTok{/}\NormalTok{pep}\OperatorTok{{-}}\DecValTok{0008}\OperatorTok{/}\NormalTok{)) that you use }\OperatorTok{**}\NormalTok{snake\_case}\OperatorTok{**}\NormalTok{ (}\BuiltInTok{all}\NormalTok{ lower}\OperatorTok{{-}}\NormalTok{case, underscore }\ControlFlowTok{for}\NormalTok{ spaces) to }\BuiltInTok{format}\NormalTok{ your variable names. The \textasciigrave{}}\OperatorTok{\textless{}}\NormalTok{value}\OperatorTok{\textgreater{}}\NormalTok{\textasciigrave{} on the right side }\KeywordTok{is}\NormalTok{ a more }\BuiltInTok{complex}\NormalTok{ story, }\ImportTok{as}\NormalTok{ it can be hard}\OperatorTok{{-}}\NormalTok{coded (}\ImportTok{as} \KeywordTok{in}\NormalTok{ example above), computed using other variables }\KeywordTok{or}\NormalTok{ the same variable, returned by a function, etc.}

\NormalTok{Using variables means that you can concentrate what corresponding values }\OperatorTok{**}\NormalTok{mean}\OperatorTok{**}\NormalTok{ rather than worrying about what these values are. For example, the }\BuiltInTok{next}\NormalTok{ time you need to compute something based on number of character}\StringTok{\textquotesingle{}s legs (e.g., how many pairs of shoes does a character need), you can compute it based on current value of \textasciigrave{}number\_of\_legs\textasciigrave{} variable rather than assume that it is \textasciigrave{}1\textasciigrave{}. }
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\KeywordTok{[Balls]}
\DataTypeTok{    x }\OtherTok{=}\StringTok{ }\FloatTok{0.1}\StringTok{, }\FloatTok{0.2}
\DataTypeTok{    y }\OtherTok{=}\StringTok{ }\FloatTok{0.2}\StringTok{, }\FloatTok{0.1}
\DataTypeTok{    size }\OtherTok{=}\StringTok{ }\FloatTok{0.01}\StringTok{, }\FloatTok{0.02}
...
Today we will stat developing a \emph{Flappy Bird} game. You control a bird that must fly through the openings in the obstacles but your only action is to ``flap the wings'' in order to counteract the effect of gravity.
...
In our game, the player can lose either if they hit an obstacle (we do not have any yet) or if the bird drops below the ground level, i.e., the bottom edge of the window. Create a new method \texttt{is\_airbourne()} that will return \texttt{True} if \texttt{y} position of the bird is above \texttt{-1} (note, you do not need an explicit \texttt{if} for this, nor do you need to write \texttt{True} or \texttt{False} anywhere, think how this can be done without these).

In the main loop add the check for \texttt{bird.is\_airbourne()} condition so that it continues until player presses \texttt{"escape"} or the bird hits the ground.

Add \texttt{is\_airbourne} method to \texttt{FlappyBird} class.
...
Turn \texttt{is\_airbourne} into a property.

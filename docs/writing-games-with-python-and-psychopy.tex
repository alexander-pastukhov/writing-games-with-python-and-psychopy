...
\CommentTok{\#\textgreater{} \textless{}generator object iterator\_fun at 0x000001BD1CBDC6D0\textgreater{}}
...
You can still access it via its mangled name\texttt{object.\_\textless{}ClassName\textgreater{}\textless{}hidden\ attribute\ name\textgreater{}}:
...
Start by creating an \texttt{AlienArmada} class, below the \texttt{Alien} class. Our initial version will have a compulsory constructor, \texttt{spawn()}, and \texttt{draw()} methods plus \texttt{win}, \texttt{\_\_pos} (a tuple with the position within the window) and \texttt{aliens} (a list with alien objects) attributes. Let us go through methods implementation.

We will create and place aliens in a separate method \texttt{spawn()}, so our constructor will be very simple. You need a \href{https://psychopy.org/api/visual/index.html}{Window} for creating \texttt{Alien} objects and a dictionary with armada-specific settings. You already specified filenames for aliens, so add an \texttt{"Initial\ position"} setting (center of the armada, I've picked \texttt{{[}0,\ 0.5{]}} but you can always optimize it later on), \texttt{"Grid\ size"} (number of aliens horizontally and vertically, I've used \texttt{{[}7,\ 3{]}}), and \texttt{Grid\ size\ {[}norm{]}} (distance between aliens in ``norm'' units, I've picked 0.2). Store \href{https://psychopy.org/api/visual/index.html}{Window} into a \texttt{win} attribute, settings into \texttt{settings} plus store the initial position of the armada in a \texttt{\_\_pos} attribute. Why the \texttt{\_\_} in front? Because we want it to be a private with a virtual \texttt{pos} attribute taking its place for public relations (more on that later). In addition, initialize \texttt{aliens} attribute to an empty list (again, all attributes must be initialized in the constructor, even if to an empty list or \texttt{None}), and call \texttt{spawn()} method.

In the \texttt{spawn()} create aliens on a grid based on your \texttt{"Grid\ size"} and \texttt{"Grid\ step\ {[}norm{]}"} settings, so they are centered at \texttt{AlienArmada}'s position (\texttt{\_\_pos}). All aliens go to \texttt{aliens} attribute, of course. Remember that they need a list of file names for visuals but you have it covered as you have them in the \texttt{settings} attribute. It should look like this:

\begin{center}\includegraphics[width=8.89in]{images/alien-armada} \end{center}

In the draw \texttt{draw()} method, you simply draw all aliens.
...
Now we need to create \texttt{Laser} class (in a separate file). Eventually, when a player presses left mouse button, the laser should appear at the location just above the ship and fly up with a certain speed. Let us take care of the first step. Create \texttt{Laser} class as a descendant of \href{https://psychopy.org/api/visual/imagestim.html\#psychopy.visual.ImageStim}{ImageStim} (just like our \texttt{Spaceship} and \texttt{Alien} classes). It should take \texttt{win}, \texttt{pos} and \texttt{speed} as arguments and call the ancestor constructor. The \texttt{pos} argument will hold the position of the ship but the laser must appear \emph{above} it, so you need to adjust \texttt{pos{[}1{]}} for that. Hint, you can get window height from \texttt{win.size{[}1{]}} and you know that sprites are 32 pixels high. The algebra should be straightforward. The \texttt{speed} is in \emph{norm units per second} and be define in settings (I've picked 1 norm / second, so it takes 2 seconds for the shot to fly across the screen). You need to save the \texttt{speed} value in an attribute for later use.
...
We do not need our lasers once they fly of the screen, so we should remove them. First, we need to know which \texttt{Laser} shot has expired (is off the screen). For this, modify the \texttt{Laser} class by adding a \protect\hyperlink{computed-attribute-property}{computed property} \texttt{expired} that is \texttt{True} if the shot is above the upper edge of the window.

Next, add a \texttt{cleanup()} method to the \texttt{LaserGun} class that will remove all \texttt{expired} shots. This is both simple and tricky! Conceptually simple: you loop over \texttt{lasershots} and delete any object that is \texttt{expired}. You can do it via list comprehension (deleting objects by not including them in the updated list) but for didactic reasons we will use \href{https://docs.python.org/3/tutorial/datastructures.html\#the-del-statement}{del} instead. If you have a list and you want to delete a second element, you write
...
Add \texttt{cleanup()} call to the main loop right after the \texttt{fly()} to automatically delete expired shots. Once you fired a few shots and they are all off the screen, put a breakpoint and check that the \texttt{.lasershots} is empty (you \texttt{cleanup()} works as it should).

Add \texttt{expired} property to \texttt{Laser}.
Add \texttt{cleanup()} method to \texttt{LaserShot}.
Use \texttt{cleanup()} in \emph{code08.py}.

...
#> <generator object iterator_fun at 0x000000002A63C820>
...
The `time_to_next_target_generator()` function should take a tuple of two float values, which define shortest and longest allowed delays, as a parameter and _yield_ a [random number](https://docs.python.org/3/library/random.html#random.uniform) within this range in an _endless_ loop. We need the endless loop (`while True:` will do) because we do not know how many values we will need, so we just generate as many as needed.

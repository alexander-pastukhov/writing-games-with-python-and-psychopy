...
Today we will stat developing a _Flappy Bird_ game. You control a bird that must fly through the openings in the obstacles but your only action is to "flap the wings" in order to counteract the effect of gravity. 
...
::: {.rmdnote .program}
Turn `is_airbourne` into a property.<br/>

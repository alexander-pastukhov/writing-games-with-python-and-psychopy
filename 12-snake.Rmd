...
Next, let us create a method that maps a position on the grid to a position in the window. This way, we can think about position of the snake or apples in terms of the grid but draw them in window coordinates. Create a new method `grid_to_win()` in the `GridWindow` class that takes a tuple of integers `(x_index, y_index)` with grid position (I called this argument `ipos`) and returns a tuple of floats `(x_pos, y_pos)` with the coordinates of the _center_ of the square in _window_ coordinates. Use (list comprehension)[#list-comprehension] to simplify the code. 
